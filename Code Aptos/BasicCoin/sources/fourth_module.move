...
module named_addr::basic_coin2 {
  ...
  /// Struct representing the balance of each address. We must have this struct to get Balance
  struct Balance has key {
    coin: Coin
  }

  /// Publish an empty balance resource under `account`'s address. Since this resouces is needed to receive coins through minting or transfering, this function must be called before minting or transferring to the account.
  public fun publish_balance(account: &signer) {
    // Check that `account` doesn't already have a `Balance` resource. Because one account can only have one value of Resouce Type Balance
    let empty_coin = Coin { value: 0 };
    assert!(!exists<Balance>(signer::address_of(account)), EALREADY_HAS_BALANCE);
    move_to(account, Balance { coin: empty_coin });
  }

  /// Initialize this module. Only the owner of the module can mint coin
  ...
  /// Returns the balance of `owner`. Use function to get value from global storage to read from it
